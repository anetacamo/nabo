...
export function Card({ post }: CardProps) {
  ...
  return (
    ...
    >
      <div className={`${styles.image} border-bottom-${getColor(post.type)}`}>
        <Image
          src={imageSrc}
          alt={post?.title}
          layout="fill"
      ...
      {post?.type && (
        <p
          className={`${styles.type} border-${getColor(post.type)} ${getColor(
            post.type
          )} bg-black`}
        >
          {post?.supertag && post.supertag} {post?.type.split(",")[0].trim()}
        </p>
      )}
